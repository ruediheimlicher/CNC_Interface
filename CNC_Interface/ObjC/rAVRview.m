...
// Funktion zum Normalisieren eines Vektors
Vekt normalize(Vekt p) {
    double length = sqrt(p.x * p.x + p.y * p.y);
   Vekt result = { p.x / length, p.y / length };
    return result;
}

// Funktion zum Berechnen der Winkelhalbierenden
Vekt bisector(Vekt prev, Vekt curr, Vekt next) 
{
    // Vektoren zu den Nachbarpunkten berechnen
   Vekt v1 = { prev.x - curr.x, prev.y - curr.y };
   Vekt v2 = { next.x - curr.x, next.y - curr.y };

    // Vektoren normalisieren
    v1 = normalize(v1);
    v2 = normalize(v2);

    // Normalisierte Vektoren addieren
   Vekt bisector = { v1.x + v2.x, v1.y + v2.y };

    // Ergebnis normalisieren (Richtung der Winkelhalbierenden)
...
- (void)microAktion:(NSNotification*)note
{
    ...
    NSLog(@"AVR microAktion micro: %d",micro);
...
- (NSArray*)stopFunktion:(NSArray*)koordinatentabelle outletdaten: (NSDictionary*)outletdic
{
   ...
   NSLog(@"stopFunktion zoomfaktor: %2.2f",zoomfaktor);
   int micro = 1;
   if([outletdic objectForKey:@"micro"])
   {
      micro = [[outletdic objectForKey:@"micro"] intValue];
   }
   ...
   if([outletdic objectForKey:@"ramp"])
   {
      ramp = [[outletdic objectForKey:@"ramp"] intValue];
   }
   NSLog(@"stopFunktion ramp: %d",ramp);
   
   float rampfaktor = 2;
   if([outletdic objectForKey:@"rampfaktor"])
   {
      rampfaktor = [[outletdic objectForKey:@"rampfaktor"] floatValue];
   }
   int timerintervall = 196;
   
   if([outletdic objectForKey:@"timerintervall"])
   {
      timerintervall = [[outletdic objectForKey:@"timerintervall"] intValue];
   }
   
   float abbrand = 0;
   if([outletdic objectForKey:@"abbrand"])
   {
      abbrand = [[outletdic objectForKey:@"abbrand"] floatValue];
   }
   int mitabbrand = 0;
   if([outletdic objectForKey:@"mitabbrand"])
   {
      mitabbrand = [[outletdic objectForKey:@"mitabbrand"] intValue];
   }

   NSLog(@"stopFunktion mitabbrand: %d abbrand: %2.2f",mitabbrand, abbrand);
   ...
   for (i=0;i<[koordinatentabelle count]-1;i++)
   {
      ...
      if (mitabbrand && [tempPrevDic objectForKey:@"abrax"]) //
      {
         // Seite A
         //NSLog(@"Seite A prev abbrand i: %d ",i);
      ...
      {
         // Seite A
         //NSLog(@"Seite A prev kein abbrand i: %d ",i);
      ...
      if (mitabbrand && [tempNowDic objectForKey:@"abrax"])
      {
         // Seite A
         //NSLog(@"Seite A now abbrand i: %d ",i);
      ...
      {
         // Seite A
         NSLog(@"Seite A now kein abbrand i: %d ",i);
      ...
      [tempDic setObject:[NSNumber numberWithFloat:ramp] forKey:@"ramp"];
      
      [tempDic setObject:[NSNumber numberWithFloat:rampfaktor] forKey:@"rampfaktor"];
      
      [tempDic setObject:[NSNumber numberWithInt:timerintervall] forKey:@"timerintervall"];
  ...
  // fprintf(stderr,"rev\n");
   ...
   for(int i=0;i<revKoordinatenTabelle.count;i++)
   {
      ...
      float abrax = [[[revKoordinatenTabelle objectAtIndex:i]objectForKey:@"ax"]floatValue];
      float abray = [[[revKoordinatenTabelle objectAtIndex:i]objectForKey:@"ay"]floatValue];
      
      float pwm = [[[revKoordinatenTabelle objectAtIndex:i]objectForKey:@"pwm"]floatValue];
...
- (IBAction)reportOberkanteAnfahren:(id)sender
{
   ...
   [CNC setSpeed:14];
   [CNC setredpwm:[red_pwmFeld floatValue]*[DC_PWM intValue]];
   [CNC setfullpwm:[DC_PWM intValue]];
...
- (NSArray*)RumpfelementmitDic:(NSDictionary*)rumpfteildic // von Taste Teil
{
...
- (NSArray*)LibProfileingabeFunktion:(NSDictionary*)eingabeDic
{
   ...
   NSDictionary* outletdaten = [rHotwireViewController cncoutletdaten];
   //NSLog(@"LibProfileingabeFunktion outletdaten; %@",outletdaten);
   if([outletdaten objectForKey:@"pwm"])
   {
      full_pwm = [[outletdaten objectForKey:@"pwm"]intValue];
   }
   if([outletdaten objectForKey:@"redpwm"])
   {
      red_pwm = [[outletdaten objectForKey:@"redpwm"]floatValue];
   }
   
   int abbrand = 0;
   
    if ([eingabeDic objectForKey:@"mitabbrand"] )
   {
      abbrand = 1;
   ...
   /*
    ...
    // Fehler im Einstich: 14 neu eingesetzt
    11, 12: Einstich
    14: Holmeinstich
   ...
   */
   ...
   /*
    ...
    30:  Unterseite, rueckwaerts eingesetzt
    40:  Nasenleisteauslauf
    50: Sicherheitsschnitt nach oben (in Enleisteneinlaufarray)
    */
  ...
  // NSLog(@"profil1array: ");
   ...
   if (!(mitOberseite && mitUnterseite))// &&  mitEinlauf) // Nur Ober- ODER Unterseite
   {
      ...
      if(einlaufok)
      {
         ...
         for(k=0;k<anz;k++)
         {
            ...
            int temppwm = [[[EndleistenEinlaufArrayA objectAtIndex:k]objectAtIndex:2]intValue];
            ...
            /*
             ...
             int temppwm = [[[EndleistenEinlaufArrayA objectAtIndex:k]objectAtIndex:2]floatValue]*full_pwm;
             
             [tempZeilenDic setObject:[NSNumber numberWithInt:red_pwm*full_pwm] forKey:@"pwm"];
             //NSLog(@"EndleistenEinlaufArrayA pwm: %2.2f",temppwm);
             
             }
             else
             {
             [tempZeilenDic setObject:[NSNumber numberWithInt:full_pwm] forKey:@"pwm"];
             }
             */
         ...
         NSLog(@"LibProfileingabeFunktion NACH endleistenarray: LibKoordinatenTabelle");
         for (int i=0;i<EndleistenEinlaufArrayA.count;i++)
         {
      ...
      for (int i=0;i<LibKoordinatenTabelle.count;i++)
      {
         ...
         float bx = [[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"bx"]floatValue];
         float by = [[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"by"]floatValue];
         int temppwm = [[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"pwm"]intValue];
         fprintf(stderr,"%d \t%2.4f \t  %2.4f \t  %2.4f \t %2.4f \t %d\n",i,ax,ay,bx,by,temppwm);
      }
   ...
   if (mitUnterseite)
   {
     ...
     // NSLog(@"redUnterseiteArray count: %d",redUnterseiteArray.count);
      
      nasenindexA = redUnterseiteArray.count;
   ...
   if (!(mitOberseite&&mitUnterseite) && mitAuslauf)
   {
      ...
      for(l=1;l<[NasenleistenAuslaufArray count];l++)
      {
         ...
         int temppwm = [[[NasenleistenAuslaufArray objectAtIndex:l]objectAtIndex:2]intValue];
         if (abbrand)
         {
         ...
         /*
          ...
          //float temppwm = [[[NasenleistenAuslaufArray objectAtIndex:l]objectAtIndex:2]floatValue]*origpwm;
          //NSLog(@"NasenleistenAuslaufArray pwm: %2.2f",temppwm);
          [tempZeilenDic setObject:[NSNumber numberWithInt:red_pwm] forKey:@"pwm"];
 ...
 //     [lastdic setObject:[NSNumber numberWithFloat:25]forKey:@"by"];
      ...
      float minyA = MAXFLOAT;
      float maxyA = 0;
      float minyB = MAXFLOAT;
      float maxyB = 0;
      float maxxA = 0;
      float maxxB = 0;
      int nasenindexA = 0;
      ...
      int wasserunterkiel = 8;
      
      for (int i=0;i<LibKoordinatenTabelle.count;i++)
      {
         if([[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"ay"]floatValue] < minyA)
         {
            minyA = [[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"ay"]floatValue];
         }
         if([[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"ay"]floatValue] > maxyA)
         {
            maxyA = [[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"ay"]floatValue];
         }
         
         if([[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"by"]floatValue] < minyB)
         {
            minyB = [[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"by"]floatValue];
         }
         if([[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"by"]floatValue] > maxyB)
         {
            maxyB = [[[LibKoordinatenTabelle objectAtIndex:i]objectForKey:@"by"]floatValue];
         }
      ...
      // [LibKoordinatenTabelle objectAtIndex:0]setObject:
      ...
      [firstDic setObject:[firstDic objectForKey:@"ax"]forKey:@"abrax"];
      ...
      [firstDic setObject:[firstDic objectForKey:@"ax"]forKey:@"abrax"];
      [firstDic setObject:[firstDic objectForKey:@"ay"]forKey:@"abray"];
      
      [firstDic setObject:[firstDic objectForKey:@"bx"]forKey:@"abrbx"];
      [firstDic setObject:[firstDic objectForKey:@"by"]forKey:@"abrby"];
 ...
 //     [LibKoordinatenTabelle insertObject:firstDic atIndex:0];
      ...
      //fprintf(stderr,"LibKoordinatenTabelle nach addabbrand: \n");
      for (int i=0;i<LibKoordinatenTabelle.count;i++)
      {
         [LibKoordinatenTabelle[i] setObject:[NSNumber numberWithInt:i]forKey:@"index"];
         NSMutableDictionary* tempdic = [NSMutableDictionary dictionaryWithDictionary:LibKoordinatenTabelle[i]];
      ...
      if(mitEinstrich)
      {
         ...
         float startxA = [[[LibKoordinatenTabelle objectAtIndex:von]objectForKey:@"ax"]floatValue];
         float nasenxA = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA]objectForKey:@"ax"]floatValue];
         
         //NSLog(@"Nasenpunkt fuer Einstich");
         for (int i=0;i<LibKoordinatenTabelle.count;i++)
         {
         ...
         float einstichxA0 = startxA + 6*oberseiteabschnittA; 
         float einstichxA1 = startxA + 10*oberseiteabschnittA;
         float einstichxA2 = startxA + 10*oberseiteabschnittA; // nicht verwendet
         float einstichxA3 = startxA + 11*oberseiteabschnittA; // nicht verwendet
         
         
         
         int einstichxArray[ANZAHLEINSTICHE] = {einstichxA0,einstichxA1,einstichxA2,einstichxA3}; // 
         ...
         for (int i = 0;i < ANZAHLEINSTICHE-2; i++) // nur noch 3 einstiche
         {
           ...
           // NSLog(@"i: %d einstich: %d",i,einstich);
            ...
            //fprintf(stderr,"raw einstich: %d \t ax: %2.2f\tay: %2.2f\tbx: %2.2f\tby: %2.2f pwm: %.2f\n",einstich,[[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"ax"]floatValue],[[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"ay"]floatValue],[[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"bx"]floatValue],[[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"by"]floatValue],[[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"pwm"]floatValue]);
            
            float ax = [[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"ax"]floatValue];
            float ay = [[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"ay"]floatValue];
            float bx = [[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"bx"]floatValue];
            float by = [[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"by"]floatValue];
            float p = [[[LibKoordinatenTabelle objectAtIndex:einstich]objectForKey:@"pwm"]floatValue];
            //fprintf(stderr,"ax: %2.2f\tay: %2.2f\tbx: %2.2f\tby: %2.2f pwm: %.2f\n",ax,ay,bx,by,p);
            ...
            prevbx = [[[LibKoordinatenTabelle objectAtIndex:einstich-1]objectForKey:@"bx"]floatValue];
            prevby = [[[LibKoordinatenTabelle objectAtIndex:einstich-1]objectForKey:@"by"]floatValue];
            p = [[[LibKoordinatenTabelle objectAtIndex:einstich-1]objectForKey:@"pwm"]floatValue];
            //fprintf(stderr,"revax: %2.2f\tprevay: %2.2f\tprevbx: %2.2f\tprevby: %2.2f\tpwm: %.2f\n",prevax,prevay,prevbx,prevby,p);
            ...
            p = [[[LibKoordinatenTabelle objectAtIndex:einstich+1]objectForKey:@"pwm"]floatValue];
            //fprintf(stderr,"nextax: %2.2f\tnextay: %2.2f\tnextbx: %2.2f\tnextby: %2.2f \tpwm: %.2f\n",nextax,nextay,nextbx,nextby,p);
            ...
            float einstichfaktor = 0;
            
            if(einheitsvektor > 0.02) // noch nicht in line
            {
               //NSLog(@"einheitsvektor > 0.02 index: %d",i);
               einstichfaktor = einstichtiefe / einheitsvektor;
            ...
            else // fast in line -> senkrechte zu prevax,y zu ax,y suchen
            {
               ...
               //fprintf(stderr,"index: \t%d\t%2.4f\t%2.4f\t%2.4f\t%2.4f\n",einstich,ax,ay,prevax, prevay );
               winkelhalbierende[0] =   (ay - prevay);
               winkelhalbierende[1] = - (ax - prevax);
               float vektorbetrag = sqrt(winkelhalbierende[0]*winkelhalbierende[0] + winkelhalbierende[1]*winkelhalbierende[1]);
               einstichfaktor = einstichtiefe / vektorbetrag;
               winkelhalbierende[0] *= einstichfaktor;
               winkelhalbierende[1] *= einstichfaktor;
            ...
            //nasenindexA++;
            //fprintf(stderr,"red pwm einstich: %d",einstich);
            [[LibKoordinatenTabelle objectAtIndex:einstich+2]setObject:[NSNumber numberWithInt:red_pwm * full_pwm] forKey:@"pwm"];
            
         } // ANZAHLEINSTICHE
         // } // mit einstich
         
         //fprintf(stderr,"LibKoordinatenTabelle nach Einstich: \n");
         for (int i=0;i<LibKoordinatenTabelle.count;i++)
         {
            [LibKoordinatenTabelle[i] setObject:[NSNumber numberWithInt:i]forKey:@"index"];
            NSMutableDictionary* tempdic = [NSMutableDictionary dictionaryWithDictionary:LibKoordinatenTabelle[i]];
         ...
         //NSLog(@"vor ausstich nasenindexA: %d nasenindexB: %d Nasenindex: %d",nasenindexA,nasenindexB, Nasenindex);
         
         float ax = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA]objectForKey:@"ax"]floatValue];
         float  ay = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA]objectForKey:@"ay"]floatValue];
         float  bx = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA]objectForKey:@"bx"]floatValue];
         float  by = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA]objectForKey:@"by"]floatValue];
         
         
         
         float nextax = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA+1]objectForKey:@"ax"]floatValue];
         float nextay = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA+1]objectForKey:@"ay"]floatValue];
         
         float prevax = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA-1]objectForKey:@"ax"]floatValue];
         float prevay = [[[LibKoordinatenTabelle objectAtIndex:nasenindexA-1]objectForKey:@"ay"]floatValue];
         ...
         [einstichDicEnd setObject:[NSNumber numberWithFloat:full_pwm] forKey:@"pwm"];
         ...
         [ausstichDicStart setObject:[NSNumber numberWithFloat:(ax + nasenausstich)] forKey:@"ax"];
         [ausstichDicStart setObject:[NSNumber numberWithFloat:(ax + nasenausstich)] forKey:@"abrax"];
         
         
         [ausstichDicStart setObject:[NSNumber numberWithFloat:(bx + nasenausstich)] forKey:@"bx"];
         [ausstichDicStart setObject:[NSNumber numberWithFloat:(bx + nasenausstich)] forKey:@"abrbx"];
         
         [LibKoordinatenTabelle insertObject:ausstichDicStart atIndex:nasenindexA];
         
  //       bis++;

         [LibKoordinatenTabelle insertObject:ausstichDicEnd atIndex:nasenindexA];
         
         [[LibKoordinatenTabelle objectAtIndex:nasenindexA+2]setObject:[NSNumber numberWithInt:red_pwm * full_pwm] forKey:@"pwm"];
         ...
         // Nasenpunkt suchen
         //NSLog(@"vor einstich nasenindexA: %d nasenindexB: %d Nasenindex: %d",nasenindexA,nasenindexB, Nasenindex);
         ...
         float einstichfaktor = naseneinstichtiefe / einheitsvektor;
         winkelhalbierende[0] *= einstichfaktor;
         winkelhalbierende[1] *= einstichfaktor;
         ...
         [einstichDicStart setObject:[NSNumber numberWithFloat:(ax + winkelhalbierende[0])] forKey:@"abrax"];
         [einstichDicStart setObject:[NSNumber numberWithFloat:(ay + winkelhalbierende[1])+0.5] forKey:@"abray"];
         
         
         [einstichDicStart setObject:[NSNumber numberWithFloat:(bx + winkelhalbierende[0])] forKey:@"bx"];
         [einstichDicStart setObject:[NSNumber numberWithFloat:(by + winkelhalbierende[1])+0.5] forKey:@"by"];
         ...
         [LibKoordinatenTabelle insertObject:einstichDicEnd atIndex:nasenindexA];
         ...
         //   fprintf(stderr,"LibKoordinatenTabelle nach Naseneinstich: \n");
         for (int i=0;i<LibKoordinatenTabelle.count;i++)
         {
            [LibKoordinatenTabelle[i] setObject:[NSNumber numberWithInt:i]forKey:@"index"];
            NSMutableDictionary* tempdic = [NSMutableDictionary dictionaryWithDictionary:LibKoordinatenTabelle[i]];
     ...
     //    fprintf(stderr,"holm startx: \t%2.2f\t nasenx %2.2f holmabstandX: %2.2f holmindex:  %d\n",startX,nasenX,holmabstandX,holmindex);
      ...
      //   fprintf(stderr,"orig index: %d\t%2.2f\t%2.2f\t%2.2f \t%2.2f\t\n",holmindex,[holmDicStart[@"ax"]floatValue],[holmDicStart[@"ay"]floatValue],[holmDicStart[@"abrax"]floatValue],[holmDicStart[@"abray"]floatValue]);
         ...
         [holmDicEnd setObject:[NSNumber numberWithFloat:full_pwm] forKey:@"pwm"];
        ...
        // fprintf(stderr,"rev index: %d\t%2.2f\t%2.2f\t%2.2f \t%2.2f\t\n",holmindex,[holmDicStart[@"ax"]floatValue],[holmDicStart[@"ay"]floatValue],[holmDicStart[@"abrax"]floatValue],[holmDicStart[@"abray"]floatValue]);
         ...
         int i;
         //fprintf(stderr,"SegmentKoordinatenArray: \n");
         int holmrahmenindex = holmindex+1; //
         for(i=1;i<SegmentKoordinatenArray.count;i++)
         {
            NSMutableDictionary* tempdic = [NSMutableDictionary dictionaryWithDictionary: SegmentKoordinatenArray[i]];
            //fprintf(stderr,"%d\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t\n",i,[tempdic[@"ax"]floatValue],[tempdic[@"ay"]floatValue],[tempdic[@"bx"]floatValue],[tempdic[@"by"]floatValue]);
            ...
            [tempdic setObject:tempdic[@"ax"] forKey:@"abrax"];
            [tempdic setObject:tempdic[@"ay"] forKey:@"abray"];
            
            [tempdic setObject:tempdic[@"bx"] forKey:@"abrbx"];
            [tempdic setObject:tempdic[@"by"] forKey:@"abrby"];
           
            [LibKoordinatenTabelle insertObject:tempdic atIndex:holmrahmenindex++];
            //rahmenindex++;
            bis++;
         }
         ...
         [[LibKoordinatenTabelle objectAtIndex:holmrahmenindex+1]setObject:[NSNumber numberWithInt:red_pwm * full_pwm] forKey:@"pwm"];

         bis++;
         
      }
      
      //fprintf(stderr,"LibKoordinatenTabelle nach holm: \n");
      for (int i=0;i<LibKoordinatenTabelle.count;i++)
      {
         [LibKoordinatenTabelle[i] setObject:[NSNumber numberWithInt:i]forKey:@"index"];
        // NSMutableDictionary* tempdic = [NSMutableDictionary dictionaryWithDictionary:LibKoordinatenTabelle[i]];
       /*
         fprintf(stderr,"%d\t%d\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t\n",i,[tempdic[@"index"]intValue],
                 ...
                 [tempdic[@"by"]floatValue],
                 [tempdic[@"abrax"]floatValue],
  ...
  //    fprintf(stderr,"LibKoordinatenTabelle nach holm end: \n");
   ...
   for (int i=0;i<LibKoordinatenTabelle.count;i++)
   {
      ...
      int teil = [[[LibKoordinatenTabelle  objectAtIndex:i]objectForKey:@"teil"]intValue ];
      //NSLog(@"i: %d teil: %d",i,teil);
      if(teil == 11 || teil == 12 ) //Einlauf, Auslauf
      {
         //NSLog(@"abr weg i: %d",i);
         float ax = [[[LibKoordinatenTabelle  objectAtIndex:i]objectForKey:@"ax"]floatValue];
         float ay = [[[LibKoordinatenTabelle  objectAtIndex:i]objectForKey:@"ay"]floatValue];
         ...
         [[LibKoordinatenTabelle  objectAtIndex:i]setObject:[NSNumber numberWithFloat:ax] forKey:@"abrax"];
         [[LibKoordinatenTabelle  objectAtIndex:i]setObject:[NSNumber numberWithFloat:ay] forKey:@"abray"];
         [[LibKoordinatenTabelle  objectAtIndex:i]setObject:[NSNumber numberWithFloat:bx] forKey:@"abrbx"];
         [[LibKoordinatenTabelle  objectAtIndex:i]setObject:[NSNumber numberWithFloat:by] forKey:@"abrby"];
   ...
   for (int i=0;i<LibKoordinatenTabelle.count;i++)
   {
      ...
      float bx = [[tempPrevDic objectForKey:@"bx"]floatValue];
      float by = [[tempPrevDic objectForKey:@"by"]floatValue];
      float abrax = [[tempPrevDic objectForKey:@"abrax"]floatValue];
      float abray = [[tempPrevDic objectForKey:@"abray"]floatValue];
      float abrbx = [[tempPrevDic objectForKey:@"abrbx"]floatValue];
      float abrby = [[tempPrevDic objectForKey:@"abrby"]floatValue];

      
      int index = [[tempPrevDic objectForKey:@"index"]intValue];
 //    fprintf(stderr,"%d \t %d \t%2.2f \t%2.2f \t%2.2f \t%2.2f\t%2.2f \t%2.2f \t%2.2f \t%2.2f\t\n",i,index,ax,ay,bx,by,abrax,abray,abrbx,abrby);
   ...
   if (mitOberseite && mitUnterseite) // Brigt sonst Block bei Ober/Unterseite durcheinander
   {
      ...
      [[LibKoordinatenTabelle  objectAtIndex:(anz-2)]setObject:[NSNumber numberWithFloat:25] forKey:@"ay"];
      [[LibKoordinatenTabelle  objectAtIndex:(anz-2)]setObject:[NSNumber numberWithFloat:25] forKey:@"abray"];

 
      [[LibKoordinatenTabelle  objectAtIndex:(anz-2)]setObject:[NSNumber numberWithFloat:30] forKey:@"bx"];
      [[LibKoordinatenTabelle  objectAtIndex:(anz-2)]setObject:[NSNumber numberWithFloat:30] forKey:@"abrbx"];

      [[LibKoordinatenTabelle  objectAtIndex:(anz-2)]setObject:[NSNumber numberWithFloat:25] forKey:@"by"];
      [[LibKoordinatenTabelle  objectAtIndex:(anz-2)]setObject:[NSNumber numberWithFloat:25] forKey:@"abrby"];
      ...
      [[LibKoordinatenTabelle  objectAtIndex:(anz-1)]setObject:[NSNumber numberWithFloat:30] forKey:@"bx"];
      [[LibKoordinatenTabelle  objectAtIndex:(anz-1)]setObject:[NSNumber numberWithFloat:30] forKey:@"abrbx"];
  ...
  // [tempLastDic setObject:[NSNumber numberWithFloat:10.0] forKey:@"ax"];
   ...
   //NSLog(@"LibProfileingabeFunktion vor Abbrand von: %d bis: %d",von,bis);
   if (mitabbrand)
   {
      if (mitEinstrich)
      {
   ...
   {
      //NSLog(@"LibProfileingabeFunktion mitOberseite && mitUnterseite first: %@",[LibKoordinatenTabelle firstObject]);
      ...
      fprintf(stderr,"LibProfileingabeFunktion LibKoordinatenTabelle ganzes profil\n");
      for (int i=0;i<LibKoordinatenTabelle.count;i++)
      {
  ...
  // [LibProfilDic setObject: LibKoordinatenTabelle forKey:@"libkoordinatentabelle"];
   ...
   for(int i=0;i<LibKoordinatenTabelle.count;i++)
   {
      //NSLog(@"%d\t%2.2F\t%2.2F\t%2.2F\t%2.2F",i,[[[LibKoordinatenTabelle objectAtIndex:i]valueForKey:@"ax" ]floatValue],[[[LibKoordinatenTabelle objectAtIndex:i]valueForKey:@"ay" ]floatValue][[[LibKoordinatenTabelle objectAtIndex:i]valueForKey:@"abrax" ]floatValue],[[[LibKoordinatenTabelle objectAtIndex:i]valueForKey:@"abray" ]floatValue]);
...
- (void)LibProfileingabeAktion:(NSNotification*)note // nicht verwendet
{
   
   NSLog(@"AV LibProfileingabeAktion start");
   //[self KT];
...
/*   
  ...
  */ 
   ...
   if (mitUnterseite)
   {
     ...
     // NSLog(@"Unterseite einfuegen red redUnterseiteArray count: %d ",redUnterseiteArray.count);
...
- (NSArray*)blockkonfigurierenFunktion:(NSDictionary*) eingabeDic
{
   ...
   //NSLog(@"blockkonfigurierenFunktion eingabekoordinatentabelle");
   ...
   if ([eingabekoordinatentabelle count])
   {
     ...
     // float blockoberkante = [[eingabeDic objectForKey:@"blockoberkante"]floatValue];
      ...
      //[blockkoordinatentabelle addObject:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:PositionA.x],@"ax",[NSNumber numberWithFloat:PositionA.y],@"ay",[NSNumber numberWithFloat:PositionB.x],@"bx", [NSNumber numberWithFloat:PositionB.y],@"by",[NSNumber numberWithInt:index],@"index",[NSNumber numberWithInt:lage],@"lage",[NSNumber numberWithFloat:red_pwm],@"pwm",nil]];
...
- (NSArray*)blockanfuegenFunktion:(NSDictionary*)eingabeDic
{
   ...
   NSLog(@"BlockanfuegenFunktion VOR Blockkonfig eingabekoordinatentabelle");
    ...
    for (int i=0;i<eingabekoordinatentabelle.count;i++)
    {
       ...
       float abrax = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"abrax"]floatValue];
       float abray = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"abray"]floatValue];

       float bx = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"bx"]floatValue];
       float by = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"by"]floatValue];
       
       float abrbx = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"abrbx"]floatValue];
       float abrby = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"abrby"]floatValue];
    
       fprintf(stderr,"%d \t%2.4f \t  %2.4f \t  %2.4f \t %2.4f \t%2.4f \t  %2.4f \t  %2.4f \t %2.4f \n",i,ax,ay,abrax,abray,bx,by,abrbx,abrby);
    }
   ...
   //[self KT];
    ...
    NSLog(@"BlockanfuegenFunktion Blockkoordinaten"); // Rechteck
   for (int i=0;i<blockkoordinatentabelle.count;i++)
   {
      float ax = [[[blockkoordinatentabelle objectAtIndex:i]objectForKey:@"ax"]floatValue];
      float ay = [[[blockkoordinatentabelle objectAtIndex:i]objectForKey:@"ay"]floatValue];
   ...
   if ([eingabekoordinatentabelle count])// 41 el
   {
      ...
      //KoordinatenTabelle = [CNC addAbbrandVonKoordinaten:KoordinatenTabelle mitAbbrandA:10 mitAbbrandB:10  aufSeite:0 ];
      
      [self updateIndex];
       eingabekoordinatentabelle = [self updateIndexVon:eingabekoordinatentabelle];
      
      NSLog(@"reportBlockanfuegen eingabekoordinatentabelle fertig" );
      for (int i=0;i<eingabekoordinatentabelle.count;i++)
      {
         float ax = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"ax"]floatValue];
         float ay = [[[eingabekoordinatentabelle objectAtIndex:i]objectForKey:@"ay"]floatValue];
...
- (IBAction)reportAndereSeiteAnfahren:(id)sender
{
   ...
   NSLog(@"reportAndereSeiteAnfahren outletdaten: %@",outletdaten);
   NSDictionary* tempDic=[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:ANDERESEITEANFAHREN] forKey:@"usb"];
   NSNotificationCenter* nc=[NSNotificationCenter defaultCenter];
 //  [nc postNotificationName:@"usbopen" object:self userInfo:tempDic];
   if ([outletdaten valueForKey:@"pwm"])
   {
      full_pwm = [[outletdaten valueForKey:@"pwm"]integerValue];
   }
   if ([outletdaten valueForKey:@"redpwm"])
   {
      red_pwm = [[outletdaten valueForKey:@"redpwm"]integerValue];
   }

   if ([outletdaten valueForKey:@"speed"])
   {
      speed = [[outletdaten valueForKey:@"speed"]integerValue];
   }
...
- (void)goHome
{
  ...
  NSLog(@"goHome Start: Horizontal bis Anschlag");
  NSNotificationCenter* nc=[NSNotificationCenter defaultCenter];
  NSMutableArray* AnfahrtArray = [[NSMutableArray alloc]initWithCapacity:0];
...
- (void)home_Horizontal
{
   ...
   int home = 1;//[outletdaten[@"home"]intValue];
  ...
  // [tempDic setObject:[NSNumber numberWithFloat:zoomfaktor] forKey:@"zoomfaktor"];
   ...
   [tempDic setObject:[NSNumber numberWithInt:0] forKey:@"ramp"];
   ...
   [HomeSchnittdatenDic setObject:[NSNumber numberWithInt:1] forKey:@"zoom"];
   [HomeSchnittdatenDic setObject:[NSNumber numberWithInt:1] forKey:@"zoomfaktor"];
   [HomeSchnittdatenDic setObject:[NSNumber numberWithInt:1] forKey:@"zoom"];
   [HomeSchnittdatenDic setObject:[NSNumber numberWithInt:1] forKey:@"home"];
   [HomeSchnittdatenDic setObject:[NSNumber numberWithInt:1] forKey:@"micro"];
   [HomeSchnittdatenDic setObject:HomeSchnittdatenArray forKey:@"schnittdatenarray"];
   [HomeSchnittdatenDic setObject:[NSNumber numberWithInt:0] forKey:@"cncposition"];
   ...
   NSNotificationCenter* nc=[NSNotificationCenter defaultCenter];
   [nc postNotificationName:@"usbschnittdaten" object:self userInfo:HomeSchnittdatenDic]; // > ViewController
