...
*/
 ...
 //  @IBOutlet weak var PWMStepper: NSStepper!
   ...
   @IBOutlet weak var RampCheckbox:  NSButton!
   
   @IBOutlet weak var TimerIntervallFeld:  NSTextField!
   
   @IBOutlet weak var RampFaktorFeld:  NSTextField!
   //@IBOutlet weak var RampFaktorFeld:  NSTextField!
   
   
   @IBOutlet weak var StiftUp:  NSButton!
...
*/
   ...
   @objc  func setOutletdaten()
   {
      ...
      outletdaten["mitabbrand"] = AbbrandCheckbox.state as AnyObject
      outletdaten["abbrand"] = AbbrandFeld.floatValue as AnyObject
   }
...
//      startdelayTimer(startdelay: Int(TimeInterval(startdelayFeld.integerValue))) 
    ...
    @objc func StopTastefunktion() // nicht verwendet
    {
       ...
       if RampCheckbox.state == NSControl.StateValue.on
       {
          outletdaten["ramp"] = 1 as AnyObject
       }
       else
       {
...
//            print("i:\t \(n) \tkey: \(k) \twert: \t\(data ?? 8888)")
 ...
 //       var rumpfteilDic = AVR?.rumpfteilTasteFunktion(self.rumpfteilDic()  as [String: Double]);
  ...
  //     NotificationCenter.default.addObserver(self, selector:#selector(neuesElementSichernAktion(_:)),name:NSNotification.Name(rawValue: "newdata"),object:nil)
      ...
      if (hotwireplist["mitabbrand"] != nil)
      {
         AbbrandCheckbox.state = NSButton.StateValue.on
      }
      else
      {
         AbbrandCheckbox.state = NSButton.StateValue.off
      }
