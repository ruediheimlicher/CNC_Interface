      ...
      if (PListPfad)      
      {
         ...
         if ([Filemanager fileExistsAtPath:PListPfad])
         {
            ...
            if ([tempPListDic objectForKey:rumpfteilpopkey])
            {
               ...
               {
                  [Portalabstand setIntValue:[[rumpfteildic objectForKey:@"portalabstand"]intValue]];
               }
               else
               {
                  [Portalabstand setIntValue:550];
               }
...
- (NSDictionary*) rumpfteilDic
{
   ...
   [teildic setObject:[NSNumber numberWithInt:[Portalabstand intValue]] forKey:@"portalabstand"];  //
   ...
   [teildic setObject:[NSNumber numberWithInt:rumpfteil] forKey:@"teil"];
   NSLog(@"rumpfteilDic : %@",teildic);
   return teildic;
}
 
- (void)rumpfdataToPlist
{
...
*/
   ...
   float pfeilungW = (breiteAraw - breiteBraw)/elementlaenge;
   float arc=atan(pfeilungW);
   
   
   
   float breiteA =  breiteAraw + [Basisabstand floatValue] * pfeilungW;
...
// J Form down zum Radius links
   ...
   float blockbreitefloat = breiteA +  2*profileinlauf + einlaufA + auslaufA;
   //float Blockbreite = 0;
...
*/
   ...
   if ( ([Profil2RawArray count]))
   {
      ...
      for (int i=1;i<[Profil2RawArray count];i++)
      {
         ...
         // letzte Werte im RawArray
         float prevdx = [[[Profil2RawArray objectAtIndex:i-1]objectForKey:@"x"]floatValue];
         float prevdy = [[[Profil2RawArray objectAtIndex:i-1]objectForKey:@"y"]floatValue];
         
         // aktuelle Werte im RawArray
         float dx = [[[Profil2RawArray objectAtIndex:i]objectForKey:@"x"]floatValue];
         float dy = [[[Profil2RawArray objectAtIndex:i]objectForKey:@"y"]floatValue];
   ...
   */
   ...
   if (mitUnterseite)
   {
      ...
      for (index=0;index< [Profil1UnterseiteArray count];index++) // Alle Punkte abfahren
      {
         int unterseiteindex=index;
         //         if ([OberseiteCheckbox state]) // Unterseite anschliessen
...
- (IBAction)reportBlockkonfigurieren:(id)sender
{
   ...
   NSLog(@"reportBlockkonfigurieren BlockKoordinatenTabelle: %@", BlockKoordinatenTabelle);
...
- (IBAction)reportProfilOberseiteTask:(id)sender
{
   ...
   else
   {
      ...
      if ([Profil1Name length] == 0)
      {
         NSLog(@"reportProfilOberseiteTask kein Profil 1");
         return;
      }
      ...
      {
         NSLog(@"reportProfiloberseiteTask kein Profil");
